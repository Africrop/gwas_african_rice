...
function(Y=NULL,G=NULL,GD=NULL,GM=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
                ...
                SNP.P3D=TRUE,SNP.effect="Add",SNP.impute="Middle",PCA.total=0, SNP.fraction = 1, seed = 123, BINS = 20,SNP.test=TRUE, 
                    SNP.MAF=0,FDR.Rate = 1, SNP.FDR=1,SNP.permutation=FALSE,SNP.CV=NULL,SNP.robust="GLM", NJtree.group=4,NJtree.type=c("fan","unrooted"),plot.bin=10^6,
        ...
        converge=1,iteration.output=FALSE,acceleration=0,iteration.method="accum",PCA.View.output=TRUE,Geno.View.output=TRUE,plot.style="Oceanic",SUPER_GD=NULL,SUPER_GS=FALSE,CG=NULL){
...
Judge=GAPIT.Judge(Y=Y,G=G,GD=GD,KI=KI,GM=GM,group.to=group.to,group.from=group.from,sangwich.top=sangwich.top,sangwich.bottom=sangwich.bottom,kinship.algorithm=kinship.algorithm,PCA.total=PCA.total)
#if G exist turn to GD and GM
if (!is.null(G)) {hm=GAPIT.HapMap(G,SNP.effect=SNP.effect,SNP.impute=SNP.impute, Create.indicator = Create.indicator, Major.allele.zero = Major.allele.zero)
GT=hm$GT
W_NA_GD=hm$GD
GI=hm$GI
rm(hm)
SNP_taxa=as.character(GI[,1])
print(paste("GAPIT impute NA value in genome with ",SNP.impute,sep=""))
#by KNN method to impute
if (SNP.impute=="KNN")
{imputed<-impute.knn(t(as.matrix(W_NA_GD)) ,k = 10, rowmax = 0.5, colmax = 0.8, maxp = 1500, rng.seed=362436069) 
X=as.data.frame(round(t(imputed$data)))
GD=as.data.frame(cbind(GT,X))}
#by old GAPIT Imputation
if (SNP.impute=="Middle")
{X=as.data.frame(t(apply(W_NA_GD,1,function(one) GAPIT.Imputation(one,impute="Middle",GI=GI))))
GD=as.data.frame(cbind(GT,X))}
colnames(GD)=c("Taxa",SNP_taxa)
rownames(GD)=1:nrow(GD)
if (is.null(GD)) stop ("GAPIT can not calulate this genotype , please alter")
}else{GD=GD
GT=GD[,1] 
GI=GM}
GM=GI
  print("GAPIT.DP accomplished successfully for multiple traits. Results are saved")
  return (list(Y=Y,G=G,GD=GD,GM=GM,KI=KI,Z=Z,CV=CV,CV.Inheritance= CV.Inheritance,GP=GP,GK=GK,
                group.from= Judge$group.from ,group.to= Judge$group.to,group.by= group.by,DPP= DPP, name.of.trait=name.of.trait,
                kinship.cluster= kinship.cluster, kinship.group= kinship.group,kinship.algorithm= kinship.algorithm,                                                    
                ...
                sangwich.top= sangwich.top,sangwich.bottom= sangwich.bottom,QC= QC,GTindex= GTindex,LD= LD,
               ...
               method.bin= method.bin,bin.size= bin.size,bin.selection= bin.selection,
        		...
        		Geno.View.output= Geno.View.output,plot.style= plot.style,SUPER_GD= SUPER_GD,SUPER_GS= SUPER_GS,CG=CG))
...
function(G=NULL,GD=NULL,GM=NULL,KI=NULL,
  kinship.algorithm="Zhang",SNP.effect="Add",SNP.impute="Middle",PCA.total=0,seed=123, SNP.fraction =1,
...
thePCA=GAPIT.PCA(X = GD[,-1], taxa = GT, PC.number = DP$PCA.total,file.output=DP$file.output,PCA.total=DP$PCA.total)
...
DPP=100000,name.of.trait="traitname",
...
if (DP$file.output&!is.null(IC$K)) {
 print("Creating heat map for kinship...")
    pdf(paste("GAPIT.Kin.thirdPart.pdf",sep=""), width = 12, height = 12)
    par(mar = c(25,25,25,25))
    heatmap.2(as.matrix(IC$K[,-1]),  cexRow =.2, cexCol = 0.2, col=rev(heat.colors(256)), scale="none", symkey=FALSE, trace="none")
    dev.off()
    print("Kinship heat map PDF created!")
write.table(IC$K, paste("GAPIT.Kin.",DP$kinship.algorithm,".csv",sep=""), quote = FALSE, sep = ",", row.names = FALSE,col.names = FALSE)
  print("Kinship save as file")
...
Pred=SS$Pred
  GI=GWAS[,1:3]
  #GI=cbind(GWAS$SNP,GWAS$Chromosome,GWAS$Position)
  ps=GWAS$P.value
  nobs=GWAS$nobs
  maf=GWAS$maf
  if(is.na(maf[1]))  maf=matrix(.5,nrow(GWAS),1)
...
if(!is.null(IC$GD)&DP$SNP.test)
{
  ...
  if(DP$file.output) GAPIT.QQ(P.values = GWAS$P.value, name.of.trait = DP$name.of.trait,DPP=DP$DPP)
   print("Manhattan plot (Genomewise)..." )
   if(DP$file.output) GAPIT.Manhattan(GI.MP = cbind(GI[,-1],ps), name.of.trait = DP$name.of.trait, DPP=DP$DPP, plot.type = "Genomewise",cutOff=DP$cutOff,seqQTN=DP$QTN.position,plot.style=DP$plot.style,plot.bin=DP$plot.bin)

 print("Manhattan plot (Chromosomewise)..." )

  #if(file.output) GAPIT.Manhattan(GI.MP = PWIP$PWIP[,2:4], name.of.trait = name.of.trait, DPP=DPP, plot.type = "Chromosomewise",cutOff=cutOff)
 print(dim(IC$GD))
 if(DP$file.output) GAPIT.Manhattan(GI.MP = cbind(GI[,-1],ps),GD=IC$GD[,-1], CG=DP$CG,name.of.trait = DP$name.of.trait, DPP=DP$DPP, plot.type = "Chromosomewise",cutOff=DP$cutOff,plot.bin=DP$plot.bin)
...
`GAPIT.Judge`<-
function(Y=Y,G=NULL,GD=NULL,KI=NULL,GM=NULL,group.to=group.to,group.from=group.from,sangwich.top=sangwich.top,sangwich.bottom=sangwich.bottom,kinship.algorithm=kinship.algorithm,PCA.total=PCA.total){
...
if(is.null(GD)&is.null(G))stop ("GAPIT Says:GAPIT need genotype!!!")
...
function(Y,G=NULL,GD=NULL,GM=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,SNP.P3D=TRUE,GP=NULL,GK=NULL,
        ...
        QTN.position=NULL,SUPER_GD=NULL,SUPER_GS=SUPER_GS,plot.style="Beach",CG=CG){
...
#  print(cbind(as.data.frame(GT),as.data.frame(GD))[1:20,1:20])
  #if(file.output) GAPIT.Manhattan(GI.MP = PWIP$PWIP[,2:4], name.of.trait = name.of.trait, DPP=DPP, plot.type = "Chromosomewise",cutOff=cutOff)
 if(file.output) GAPIT.Manhattan(GI.MP = cbind(GI[,-1],ps),GD=GD,CG=CG, name.of.trait = name.of.trait, DPP=DPP, plot.type = "Chromosomewise",cutOff=cutOff,plot.bin=plot.bin)
...
gapitMain <- GAPIT.Main(Y=Y[,c(1,trait)],G=G,GD=GD,GM=GM,KI=KI,Z=Z,CV=CV,CV.Inheritance=CV.Inheritance,GP=GP,GK=GK,SNP.P3D=SNP.P3D,kinship.algorithm=kinship.algorithm,
                      bin.from=bin.from,bin.to=bin.to,bin.by=bin.by,inclosure.from=inclosure.from,inclosure.to=inclosure.to,inclosure.by=inclosure.by,
                        ...
                        Model.selection = Model.selection, Create.indicator = Create.indicator,
						            QTN=QTN, QTN.round=QTN.round,QTN.limit=QTN.limit, QTN.update=QTN.update, QTN.method=QTN.method, Major.allele.zero=Major.allele.zero,
                        QTN.position=QTN.position,plot.style=plot.style,SUPER_GS=SUPER_GS,CG=CG)  
...
gapitMain <- GAPIT.Main(Y=ic_Y,GD=ic_GD[,-1],GM=ic_GM,KI=ic_KI,Z=Z,CV=DP$CV,CV.Inheritance=DP$CV.Inheritance,GP=DP$GP,GK=DP$GK,SNP.P3D=DP$SNP.P3D,kinship.algorithm=DP$kinship.algorithm,
				        ...
				        group.from=DP$group.from,group.to=DP$group.to,group.by=DP$group.by,kinship.cluster=DP$kinship.cluster,kinship.group=DP$kinship.group,name.of.trait=DP$traitname,
                        file.path=DP$file.path,file.from=DP$file.from, file.to=DP$file.to, file.total=DP$file.total, file.fragment = DP$file.fragment, file.G=DP$file.G,file.Ext.G=DP$file.Ext.G,file.GD=file.GD, file.GM=file.GM, file.Ext.GD=file.Ext.GD,file.Ext.GM=file.Ext.GM, 
...
#=============================================================================================
`GAPIT3` <-
function(Y=NULL,G=NULL,GD=NULL,GM=NULL,K=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
                DPP=100000,kinship.algorithm="VanRaden",SNP.P3D=TRUE,SNP.effect="Add",
				SNP.impute="Middle",PCA.total=0, SNP.fraction = 1, seed = 123, BINS = 20,SNP.test=TRUE, 
                SNP.MAF=0,FDR.Rate = 1, SNP.FDR=1,SNP.permutation=FALSE,SNP.CV=NULL,SNP.robust="GLM",                             
                file.from=1, file.to=1, file.total=NULL, file.fragment = 99999,file.path=NULL, 
                file.G=NULL, file.Ext.G=NULL,file.GD=NULL, file.GM=NULL, file.Ext.GD=NULL,file.Ext.GM=NULL, 
                LD.chromosome=NULL,LD.location=NULL,LD.range=NULL,
                QC=TRUE,GTindex=NULL,cutOff=0.01, Model.selection = FALSE,Create.indicator = FALSE,
				QTN=NULL, QTN.round=1,QTN.limit=0, QTN.update=TRUE, QTN.method="Penalty", Major.allele.zero = FALSE,
                method.GLM="fast.lm",method.sub="reward",method.sub.final="reward",method.bin="static",output.numerical = FALSE,NJtree.group=5,NJtree.type=c("fan","unrooted"),plot.bin=10^9,
                memo="",Prior=NULL,ncpus=1,threshold.output=.01,bin.size=c(1000000),bin.selection=c(10,20,50,100,200,500,1000),
                WS=c(1e0,1e3,1e4,1e5,1e6,1e7),alpha=c(.01,.05,.1,.2,.3,.4,.5,.6,.7,.8,.9,1),maxOut=100,QTN.position=NULL,
                iteration.output=FALSE,iteration.method="accum",SUPER_GD=NULL,SUPER_GS=FALSE,CG=NULL,
		 SUPER=NULL,
         CMLM=NULL,
         Result=NULL,
		 Emma=NULL,
...
if(is.null(KI)&!is.null(K))KI=K
if(!is.null(KI)&is.null(K))K=KI
if(!is.null(KI)&!is.null(K))stop("Not both K and KI allowed")
#FarmCPU
if(is.null(FarmCPU$acceleration))FarmCPU$acceleration=0
if(is.null(FarmCPU$converge))FarmCPU$converge=1
if(is.null(FarmCPU$maxLoop))FarmCPU$maxLoop=3

#Emma
if(is.null(Emma$ngrid))Emma$ngrid=100
if(is.null(Emma$llim))Emma$llim=-10
if(is.null(Emma$ulim))Emma$ulim=10
if(is.null(Emma$esp))Emma$esp=1e-10
#CMLM
if(is.null(CMLM$group.from))CMLM$group.from=30
if(is.null(CMLM$group.to))CMLM$group.to=1000000
if(is.null(CMLM$group.by))CMLM$group.by=10
if(is.null(CMLM$kinship.cluster))CMLM$kinship.cluster="average"
if(is.null(CMLM$kinship.group))CMLM$kinship.group='Mean'
#SUPER
if(is.null(SUPER$bin.from))SUPER$bin.from=10000
if(is.null(SUPER$bin.to))SUPER$bin.to=10000
if(is.null(SUPER$bin.by))SUPER$bin.by=10000
if(is.null(SUPER$inclosure.from))SUPER$inclosure.from=10
if(is.null(SUPER$inclosure.to))SUPER$inclosure.to=10
if(is.null(SUPER$inclosure.by))SUPER$inclosure.by=10
if(is.null(SUPER$LD))SUPER$LD=0.1
#Result
if(is.null(Result$PCA.View.output))Result$PCA.View.output=TRUE
if(is.null(Result$Geno.View.output))Result$Geno.View.output=TRUE
if(is.null(Result$plot.style))Result$plot.style="Oceanic"
if(is.null(Result$file.output))Result$file.output=TRUE
if(is.null(Result$output.hapmap))Result$output.hapmap=FALSE
...
traitname=colnames(Y)[trait]
###Statistical distributions of phenotype
if(!is.null(Y) & Result$file.output)ViewPhenotype<-GAPIT.Phenotype.View(myY=Y[,c(1,trait)],traitname=traitname,memo=memo)
###Correlation between phenotype and principal components
print(paste("Processing trait: ",traitname,sep=""))
if(!is.null(memo)) traitname=paste(memo,".",traitname,sep="")
DP=GAPIT.DP(Y=Y[,c(1,trait)],G=G,GD=GD,GM=GM,KI=KI,Z=Z,CV=CV,CV.Inheritance= CV.Inheritance,GP=GP,GK=GK,
                group.from=CMLM$group.from ,group.to= CMLM$group.to,group.by=CMLM$group.by,DPP= DPP, name.of.trait=traitname,
                kinship.cluster=CMLM$kinship.cluster, kinship.group=CMLM$kinship.group,kinship.algorithm= kinship.algorithm, NJtree.group=NJtree.group,NJtree.type=NJtree.type,plot.bin=plot.bin,                                                   
                sangwich.top=SUPER$sangwich.top,sangwich.bottom=SUPER$sangwich.bottom,LD=SUPER$LD,bin.from= SUPER$bin.from,bin.to= SUPER$bin.to,bin.by= SUPER$bin.by,inclosure.from= SUPER$inclosure.from,inclosure.to= SUPER$inclosure.to,inclosure.by= SUPER$inclosure.by,
                SNP.P3D= SNP.P3D,SNP.effect= SNP.effect,SNP.impute= SNP.impute,PCA.total= PCA.total, SNP.fraction = SNP.fraction, seed = seed, 
                BINS = BINS,SNP.test=SNP.test, SNP.MAF= SNP.MAF,FDR.Rate = FDR.Rate, SNP.FDR= SNP.FDR,SNP.permutation= SNP.permutation,
                SNP.CV= SNP.CV,SNP.robust= SNP.robust,                             
                file.from= file.from, file.to=file.to, file.total= file.total, file.fragment = file.fragment,file.path= file.path, 
                file.G= file.G, file.Ext.G= file.Ext.G,file.GD= file.GD, file.GM= file.GM, file.Ext.GD= file.Ext.GD,file.Ext.GM= file.Ext.GM, 
                ngrid = Emma$ngrid, llim = Emma$llim, ulim = Emma$ulim, esp = Emma$esp,
                LD.chromosome= LD.chromosome,LD.location= LD.location,LD.range= LD.range,
                QC= QC,GTindex= GTindex,cutOff=0.01, Model.selection = Model.selection,output.numerical = output.numerical,
                Create.indicator = Create.indicator,QTN= QTN, QTN.round= QTN.round,QTN.limit= QTN.limit, QTN.update= QTN.update, QTN.method= QTN.method, Major.allele.zero = Major.allele.zero,
                method.GLM= method.GLM,method.sub= method.sub,method.sub.final= method.sub.final,
                method.bin= method.bin,bin.size= bin.size,bin.selection= bin.selection,
        		memo= memo,Prior= Prior,ncpus=1,maxLoop= FarmCPU$maxLoop,threshold.output= threshold.output,
        		WS= WS,alpha= alpha,maxOut= maxOut,QTN.position= QTN.position, converge=FarmCPU$converge,iteration.output= iteration.output,acceleration=FarmCPU$acceleration,
        		iteration.method= iteration.method,PCA.View.output=Result$PCA.View.output, 
        		output.hapmap = Result$output.hapmap, file.output= Result$file.output,Geno.View.output=Result$Geno.View.output,plot.style=Result$plot.style,SUPER_GD= SUPER_GD,SUPER_GS= SUPER_GS,CG=CG)
IC=GAPIT.IC(DP=DP)
...
ID=GAPIT.ID(DP=DP,IC=IC,SS=SS)
  ...
  #names(out$GWAS$result)=c("SNP","Chromosome","Position","mp","mc","maf","nobs")
